...
- `backend/prisma/migrations/20260721110000_phase_7_oauth_token_lifecycle/migration.sql`: adds optional access/refresh expiry metadata, encrypted refresh-token storage, and invalidation timestamps without changing existing non-expiring grants.
...
- `backend/src/routes/auth.routes.ts`: maps registration, login, authenticated GitHub linking, GitHub OAuth start/callback, and one-time OAuth exchange URLs to controller functions.
...
- `backend/src/services/oauth-token.service.ts`: encrypts GitHub access/refresh tokens with AES-256-GCM, tracks expiry, rotates expiring user tokens with optimistic concurrency protection, invalidates rejected grants safely, creates PKCE pairs, and hashes short-lived OAuth exchange codes.
...
- `backend/src/services/pilot.service.ts`: records repository-bound finding verification decisions and computes pilot readiness from distinct PR coverage, run reliability, and version-specific deterministic-rule precision.
- `backend/src/services/pilot-gate.service.ts`: keeps per-rule enforcement eligibility pure by requiring both minimum precision and minimum verified sample size.
