# Phase 6 — SNUHI Pilot Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Validate DiffGuard safely on the SNUHI repository by building pilot tracking, finding verification, precision measurement, and controlled check enablement.
...
```bash
git add -A
git commit -m "feat(pilot): Phase 6 SNUHI pilot implementation complete"
```
...
1. **Install GitHub App on SNUHI** — minimum permissions (pull_requests: read, checks: write, contents: read)
